...
- **Phase 6 built** — MCP server (`mcp/README.md`): seven tools over both transports — remote streamable HTTP mounted at `/mcp/{API_TOKEN}` for claude.ai custom connectors (secret-path perimeter; OAuth = 6b), and stdio for Claude Code/Desktop. Entries logged via Claude carry `entry_method='mcp'`.
- **Phase 5 built** — AI plate estimation: camera button → downscaled photo → `POST /api/log/estimate` (Claude vision, `ESTIMATE_MODEL`, default Haiku) → candidates matched against the local catalog → confirm-before-save sheet (adjust grams, swap matches, manual-search unmatched rows, live totals). Text "describe the meal" path on the same pipeline. Requires `ANTHROPIC_API_KEY` in Railway variables. Photos are processed, never stored.
